...
use Illuminate\Support\Facades\DB; // <-- PERUBAHAN: Ditambahkan
use Carbon\Carbon;                   // <-- PERUBAHAN: Ditambahkan

class AdminController extends Controller
{
    /**
     * PERUBAHAN: Metode baru untuk menampilkan dashboard admin.
     */
    public function dashboard()
    {
        // Mengambil data jumlah pengguna per peran
        $userCounts = User::select('role', DB::raw('count(*) as total'))
                            ->groupBy('role')
                            ->pluck('total', 'role');

        // Menyiapkan data untuk Chart.js
        $chartLabels = $userCounts->keys();
        $chartValues = $userCounts->values();

        // Mengambil data tanggal hari ini
        $todayDate = Carbon::now()->isoFormat('dddd, D MMMM Y');

        // Mengirim semua data ke view
        return view('admin.dashboard', compact('userCounts', 'chartLabels', 'chartValues', 'todayDate'));
    }
